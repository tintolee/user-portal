...
enum DispatchType {
  ...
  AmountStepSuccess = 'AmountStepSuccess',
  YourAddressStepSuccess = 'YourAddressStepSuccess',
  UpdatePhoneNumberSuccess = 'UpdatePhoneNumberSuccess',
  TransferScheduleSuccess = 'TransferScheduleSuccess',
  RecipientStepSuccess = 'RecipientStepSuccess',
  SecurityQuestionStepSuccess = 'SecurityQuestionStepSuccess',
...
const stateReducer: StateReducer = function (state, { type, data = {} }) {
  ...
  switch (type) {
    ...
    case DispatchType.SetVoucherData:
      ...
      if (amountFormData) {
        paidAmount =
          amountFormData.sendAmount + (amountFormData.rate.fee - (data.voucher?.discount || 0));
      }
    ...
    case DispatchType.AmountStepSuccess:
      if (data.amountFormData) {
        let updatedState = onStepSuccess(state, data);

        updatedState = clearSelectedRecipientIfInvalid(updatedState, data);
        updatedState = clearSecurityQuestionIfInvalid(updatedState, data);
        updatedState = updateStepsVisibleState(updatedState, data);

        return { ...updatedState, amountFormData: data.amountFormData };
      } else {
        return state;
      }
    ...
    case DispatchType.TransferScheduleSuccess:
      if (data.transferScheduleData) {
        const updatedState = onStepSuccess(state, data);

        return {
          ...updatedState,
          transferScheduleData: data.transferScheduleData
        };
      } else {
        return state;
      }
    ...
    case DispatchType.SecurityQuestionStepSuccess:
      if (data.securityQuestionFormData) {
        const updatedState = onStepSuccess(state, data);

        return {
          ...updatedState,
          securityQuestionFormData: data.securityQuestionFormData
        };
      } else {
        return state;
      }
